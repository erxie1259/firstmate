...
fm_backend_herdr_events_capable() {  # <session>
  ...
  # Use in-memory fixed-string substring tests instead of `printf | grep -Fq`.
  # A short-circuiting grep can close the pipe before the ~220KB printf finishes, causing a spurious `printf: write error: Broken pipe` diagnostic.

...
| `fm-project-mode.sh`     | Resolve a project's registered delivery posture, or with `--lane` its memory lane, from `data/projects.md` |
| `fm-merge-local.sh`      | Fast-forward a `local-only` project's local default branch after approval            |
| `fm-review-diff.sh`      | Review a crewmate branch or resolved PR head against the authoritative base          |
| `fm-gate.sh`             | Check a finished task's own claims against its actual diff, deliverables, and declared tests; read-only and never a silent pass |
| `fm-envelope-lib.sh`     | Shared reader and validator for a crewmate's optional typed terminal envelope, whose contract `fm-brief.sh` owns |
...
| `fm-pyenv-rehash-lib.sh` | Shared owner of pyenv's rehash-lock path and the structural "is a genuine rehash live?" proof |
| `fm-pyenv-shim-lock-clear.sh` | Move pyenv's orphaned rehash lock aside, or refuse while a real rehash is live; takes no arguments |

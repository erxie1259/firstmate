...
Agent registration can lag behind pane launch, so Firstmate polls the read-only agent lookup up to 20 times at 0.25 second intervals and issues the rename only once an agent is actually registered in that pane.
A pane that never registers one - a plain shell command, a test fixture - is left exactly as it was found, because renaming an agentless pane is a write whose meaning varies between Herdr releases.
The probe limit and delay are configurable via `FM_BACKEND_HERDR_AGENT_RENAME_ATTEMPTS` and `FM_BACKEND_HERDR_AGENT_RENAME_DELAY`.
...
Any rename failure warns and lets spawn, metadata publication, and supervision continue unchanged.
...
Task cleanup acquires that session lock before the task's isolated copy is returned, so a lock that stays contended for the whole wait below refuses before anything is mutated, while the copy, every durable record, and the endpoint are all intact for a plain rerun.
Every acquirer whose correctness depends on the lock - a projected spawn, a presentation recovery, and that teardown preflight - waits out one shared bounded budget derived from the longest legitimate hold, which is a projected spawn's own hold from before its workspace exists until after the harness launch, so a concurrent caller outlasts a well-behaved holder instead of refusing work that was only ever going to be slow.
That wait is minutes rather than seconds, so an acquirer that has to poll prints one stderr notice naming the measured seconds and what it is waiting for, while an acquirer whose result is discarded, such as the adapter's own best-effort pane close or an interrupted spawn's cleanup, gives up far sooner and leaves the work to the acquirer that follows.
The budget stays bounded on purpose: a genuinely wedged holder still refuses loudly rather than hanging its acquirer forever, and `bin/backends/herdr.sh` owns both budgets and their derivation beside the lock path itself.

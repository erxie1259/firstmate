...
- Helper scripts in `bin/` are plain bash by default; write one in another language only when it wraps a library of that language, as `bin/fm-memory-mcp` does for Python.

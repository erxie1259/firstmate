...
ENVELOPE_PATH="$DATA/$ID/envelope.json"
if [ -e "$ENVELOPE_PATH" ]; then
  if [ ! -f "$SCRIPT_DIR/fm-envelope-lib.sh" ] \
    || [ ! -r "$SCRIPT_DIR/fm-envelope-lib.sh" ]; then
    echo "ENVELOPE: $ID left an envelope that could not be checked - bin/fm-envelope-lib.sh is unavailable" >&2
  else
    # Load the optional reader only when the optional record exists, so an
    # absent envelope preserves teardown's previous dependency surface.
